...
/**
 * Drives the installed release app (kr.mom.probe) through onboarding using
...
class ReleaseOnboardingDriverTest {
    ...
    fun driveReleaseOnboarding() {
        ...
        launch("kr.mom.probe", "kr.mom.probe.MainActivity")

        // 1. Welcome screen (only if consent not already given).
        if (waitForText("동의해요", timeoutMs = 6_000) != null) {
            clickText("동의해요", timeoutMs = 5_000)
            clickText("이 기기에서 시작", timeoutMs = 5_000)
        }

        // 2. Child profile: name, school, level, grade.
        waitForText("자녀 정보 저장", timeoutMs = 10_000)
